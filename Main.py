...
def confirm_shutdown():
    ...
    if confirmation:
        if "deactivate" in confirmation.lower():
            speak("Goodbye, shutting down now.")
            exit(0)
        else:
            speak("I didn't catch that. Please say 'deactivate' to shut down")
            retry_confirmation = listen_command() 
            if retry_confirmation and "deactivate" in retry_confirmation.lower():
                speak("Goodbye, shutting down now.")
                exit(0)
    else:
